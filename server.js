...
app.get('/teamstats', function(req, res) {
	 var query = 'SELECT * FROM football_db;';
	db.any(query)
	    .then(function (rows) {
...
app.get('/teamstats', function(req, res) {
	var query = 'select * football_games;';
	db.any(query)
...
app.get('/teamstats', function(req, res) {
	...
	var query2 = 'Select count(game_date) as sol from football_games where game_date in (select game_date from football_games as a where home_score > visitor_score);';
	var query3 = 'Select count(game_date) as sol from football_games where game_date in (select game_date from football_games as a where home_score <= visitor_score);';
...
app.get('/player-info/select-player', function(req, res) {
	var value = req.query.id;
	var query1 = 'SELECT id, name from football_players;';
	var query2 = 'SELECT * FROM football_players where id =' + value +';';
	var query3 = 'Select count(game_date) as sol from football_games where '+value+' = ANY (Players);';
